...
spawning one for a two-minute clip wastes more than it saves. A long recording:
split the frame sequence into batches, one subagent per batch, run in parallel,
with a handful of frames' overlap between adjacent batches so an action spanning a
batch boundary doesn't get missed by both sides or double-counted by both.

Each batch subagent reads its frames (and the transcript slice covering the same
window) and returns a partial extraction in the declared mode — a segment of
steps, or a segment of the page/component shape.
